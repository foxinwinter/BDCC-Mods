...
func getProgress():
    ...
    if getFlag("PhoenixRising.Ch1Complete", false):
        result.append("Arrived at Azure Cove")
    else:
        if getFlag("PhoenixRising.Ch1_ArrivalDone", false):
            result.append("Touch down on Azure Cove")
        elif getFlag("PhoenixRising.Ch1_NightWatchDone", false):
            result.append("Finish the journey to Azure Cove")
        elif getFlag("PhoenixRising.Ch1_ShipLifeDone", false):
            result.append("Spend time with Tavi during the journey")
        elif getFlag("PhoenixRising.Ch1_MorningDone", false):
            result.append("Get to know your new ship")
        else:
            result.append("Wake up and talk to Tavi about the plan")
        return result

    if getFlag("PhoenixRising.Ch2Complete", false):
        result.append("Recruited Nova")
    else:
        if getFlag("PhoenixRising.Ch2_RecruitDone", false):
            result.append("Recruit Nova to the crew")
        elif getFlag("PhoenixRising.Ch2_MeetNovaDone", false):
            result.append("Meet Nova at Dawfort Creamery")
        elif getFlag("PhoenixRising.Ch2_ToFarmDone", false):
...
func isCompleted():
    return getFlag("PhoenixRising.Complete", false)
