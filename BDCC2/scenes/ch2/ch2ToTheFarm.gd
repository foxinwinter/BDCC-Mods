...
func _run():
    if state == "":
        ...
        saynn("[say=tavi]Huh. Not bad for a corpo resort.[/say]")
        saynn("She stretches, her tail curling behind her.")
        saynn("[say=tavi]Come on. Nova's place is on the north edge. Maybe twenty minutes on foot.[/say]")
    ...
    if state == "walk":
        ...
        saynn("[say=tavi]Weird, right? People out here just.. live. They wake up, go to work, eat dinner, go to sleep. No one's watching the clock for a guard to come by.[/say]")
    ...
    if state == "nice":
        ...
        saynn("[say=tavi]Yeah. It is.[/say]")
        saynn("She bumps her shoulder against yours as you walk.")
        saynn("[say=tavi]Maybe we can have this, one day. When all the shit is over. A little place somewhere quiet. No AlphaCorp. No Syndicate. Just.. this.[/say]")
    ...
    if state == "wont_last":
        ...
        saynn("[say=tavi]Yeah. I know.[/say]")
        saynn("She picks up the pace.")
        saynn("[say=tavi]Let's just.. get to Nova. Figure out the next move before the universe reminds us we're not on vacation.[/say]")
    ...
    if state == "approach_farm":
        ...
        saynn("Tavi chuckles.")
        saynn("[say=tavi]She made a petting zoo. Of course she did.[/say]")
...
func _react(_action, _args):
    if _action == "end_scene":
        GM.main.setFlag("BDCC2.Ch2_ToFarmDone", true)
        endScene()
        return
    setState(_action)
